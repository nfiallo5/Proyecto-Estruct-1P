...
public T remove(int indice){
	...
	if(indice == 0){ //Si el indice es el primer nodo
		...
		data = actual.getData();
		NodoDoble<T> anterior = actual.getPrev(); //Se obtiene el nodo anterior al buscado
		anterior.setSigt(actual.getSigt()); //Se define el nuevo 'NEXT' como el next nodo del que se va a remover 
		NodoDoble<T> posterior = actual.getSigt(); //Se obtiene el nodo posterior al buscado
